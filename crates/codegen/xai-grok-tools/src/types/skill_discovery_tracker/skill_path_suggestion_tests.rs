...
fn suggests_unique_registered_path_for_wrong_root() {
    let manager = seeded_manager(vec![skill(
        "code-review",
        "/home/user/.opengrok/skills/code-review/SKILL.md",
    )]);
    ...
    assert_eq!(
        ...
        Path::new("/home/user/.opengrok/skills/code-review/SKILL.md")
    );
}

#[test]
fn suggests_nothing_for_ambiguous_disabled_non_skill_or_exact_requests() {
    let mut retired = skill("retired", "/home/user/.opengrok/skills/retired/SKILL.md");
    retired.enabled = false;
    let manager = seeded_manager(vec![
        skill("review", "/repo/.opengrok/skills/review/SKILL.md"),
        skill("review", "/home/user/.opengrok/skills/review/SKILL.md"),
        retired,
        skill("solo", "/home/user/.opengrok/skills/solo/SKILL.md"),
    ]);
    ...
    for requested in [
        ...
        "/home/user/.opengrok/skills/solo/SKILL.md",
    ] {
        assert!(
...
fn requested_path_among_same_named_registrations_is_ambiguous() {
    ...
    for requested in [
        "/repo/.opengrok/skills/review/SKILL.md",
        "/home/user/.opengrok/skills/review/SKILL.md",
    ] {
        assert!(
            manager.suggest_skill_path(Path::new(requested)).is_none(),
            "{requested}"
...
fn includes_model_disabled_and_held_conditional_skills() {
    let mut model_disabled = skill("manual", "/home/user/.opengrok/skills/manual/SKILL.md");
...
fn baseline_reload_updates_lookup_but_not_snapshot_names() {
    ...
    manager.seed(
        None,
        None,
        vec![skill("initial", "/repo/.opengrok/skills/initial/SKILL.md")],
        None,
        None,
        None,
    );
    ...
    manager.update_startup_baseline(vec![skill(
        "reloaded",
        "/repo/.opengrok/skills/reloaded/SKILL.md",
    )]);
...
fn reload_disabling_a_discovered_skill_stops_suggesting_it() {
    let path = "/repo/.opengrok/skills/review/SKILL.md";
...
fn reload_moving_a_skill_suggests_only_the_current_registration() {
    ...
    let mut manager = seeded_manager(vec![skill(
        "review",
        "/repo/old/.opengrok/skills/review/SKILL.md",
    )]);
    manager.update_startup_baseline(vec![skill(
        "review",
        "/repo/new/.opengrok/skills/review/SKILL.md",
    )]);
    let suggestion = manager
        .suggest_skill_path(Path::new("/wrong/root/review/SKILL.md"))
        .unwrap();
    assert_eq!(
        suggestion.display_path,
        Path::new("/repo/new/.opengrok/skills/review/SKILL.md")
    );
...
fn rewrites_worktree_paths_to_display_cwd_but_preserves_external_paths() {
    let mut manager = seeded_manager(vec![
        skill("review", "/real/worktree/.opengrok/skills/review/SKILL.md"),
        skill("external", "/home/user/.opengrok/skills/external/SKILL.md"),
    ]);
    ...
    assert_eq!(
        manager
            ...
            .unwrap()
            .display_path,
        Path::new("/display/project/.opengrok/skills/review/SKILL.md")
    );
    assert_eq!(
        manager
            ...
            .unwrap()
            .display_path,
        Path::new("/home/user/.opengrok/skills/external/SKILL.md")
    );
}
