    ...
    I: IntoIterator<Item = &'a ModelEntry>,
{
    ...
    // Only xAI-provider BYOK advertises `xai.api_key` (provider isolation).
    // First-party env is gated by the validity probe; BYOK still advertises
    // without a probe.
    let has_byok = models.into_iter().any(|model| {
