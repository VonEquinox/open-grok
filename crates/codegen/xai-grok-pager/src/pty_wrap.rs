...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // Advertise OSC 52 sink + optional local appearance to the wrapped program
    // (and anything it spawns, e.g. a remote `open-grok` over SSH). See
    // [`apply_wrap_child_env`]: `GROK_*` for local children, `LC_GROK_*` for
    // default OpenSSH `SendEnv`/`AcceptEnv LANG LC_*` forwarding.
